...
import com.heeee.plant_care_app.repository.PlantTypeWikiRepository
...
class PlantService(
    ...
    fun createPlant(userId: Long, request: PlantCreateRequest): PlantDetailResponse {
        ...
        val newPlant = Plant(
            ...
            // 마지막으로 물 준 날짜가 없으면, 등록일(오늘)을 기준으로 설정합니다.
            lastWateredDate = request.lastWateredDate ?: LocalDate.now(),
    ...
    fun updatePlant(userId: Long, plantId: Long, request: PlantUpdateRequest): PlantDetailResponse {
        ...
        plant.startDate = request.startDate ?: plant.startDate
        plant.lastWateredDate = request.lastWateredDate ?: plant.lastWateredDate
        ...
        } else if (newPlantType == null) {
            // plantType이 null로 변경된 경우, 정보 초기화
            plant.wateringCycleDays = null
            plant.description = null
            plant.careInfo = null
        }
